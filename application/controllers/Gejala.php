...
class Gejala extends CI_Controller
{
	...
	private function loadView($showModal = "index")
	{
		$data = [
			'title' => 'Gejala',
			'gejala' => $this->M_gejala->get()->result(),
		...
		$this->load->view('content/admin/gejala/index', $data);
	...
	public function add()
	{
		$this->form_validation->set_rules('kodegejala', 'Kode Gejala', 'required|trim|min_length[1]|max_length[8]|is_unique[gejala.kodegejala]', [
			'required' => 'Kode Gejala harus diisi!',
			'min_length' => 'Kode Gejala terlalu pendek!',
			'max_length' => 'Kode Gejala terlalu panjang!',
			'is_unique' => 'Kode Gejala sudah digunakan!',
		]);
		$this->form_validation->set_rules('namagejala', 'Nama Gejala', 'required|trim', [
			'required' => 'Nama Gejala harus diisi!',
		]);
		$this->form_validation->set_rules('ketgejala', 'Keterangan Gejala', 'required|trim', [
			'required' => 'Keterangan Gejala harus diisi!',
		]);
		$this->form_validation->set_rules('kodepenyakit', 'Hama & Gejala', 'required|trim', [
			'required' => 'Hama & Penyakit harus diisi!',
		]);
